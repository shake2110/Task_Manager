...
function updateTaskUrls(projectId: string, tasks: IProjectDigestTask[]) {
  ...
  for (const task of tasks) {
    task.url = `${baseUrl}/taskeye/projects/${projectId}?tab=tasks-list&task=${task.id}`;
  }
}
...
function updateMetadata(project: IProjectDigest, subscriberName: string) {
  ...
  project.settings_url = `${getBaseUrl()}/taskeye/settings/notifications`;
  project.project_url = `${getBaseUrl()}/taskeye/projects/${project.id}?tab=tasks-list`;
}
