import TaskEyeControllerBase from "../controllers/taskeye-controller-base";
...
/** HandleExceptions can only be used with an instance of TaskEyeControllerBase. */
export default function HandleExceptions(options?: IExceptionHandlerConfig) {
  const opt = mergeWithDefaults(options);
  return (target: any, key: string, descriptor: PropertyDescriptor) => {
    if (!(target.prototype instanceof TaskEyeControllerBase))
      throw new Error("@HandleExceptions can only be used with an instance of TaskEyeControllerBase.");
