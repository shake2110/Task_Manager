...
const TEMPLATES_BASE = "../../taskeye-email-templates";
...
router.get("/task-moved-to-done", (req: IWorkLenzRequest, res: IWorkLenzResponse) => {
  ...
  const task = {
    name: "Soloman's Island Project - SIM Registration + Central DB + KYC",
    members: "Dinindu, Dinindu Wanniarachchi, Pasindu Ishan",
    url: "http://localhost:4200/taskeye/projects/6de9c9df-2193-4212-8d64-9dfc36a83ed2?tab=tasks-list&task=f8b3fc45-a28b-4d8f-985e-9e43f8577aa8",
...
router.get("/project-daily-digest", (req: IWorkLenzRequest, res: IWorkLenzResponse) => {
  const sampleData: IProjectDigest = {
    ...
    project_url: "/",
    subscribers: [],
    summary: `Here's the "TaskEye" summary | Ceydigital`,
    today_completed: [
      {
        id: "abc123",
        name: "Sample Task",
