...
export default class TaskCommentsController extends TaskEyeControllerBase {
  ...
  private static async sendMail(config: IMailConfig) {
    ...
    const taskUrl = `${getBaseUrl()}/taskeye/projects/${config.projectId}?tab=tasks-list&task=${config.taskId}&focus=comments`;
    const settingsUrl = `${getBaseUrl()}/taskeye/settings/notifications`;
