...
export const AdminGuard = memo(({ children }: GuardProps) => {
  ...
  if (!isAdmin) {
    return <Navigate to="/taskeye/unauthorized" />;
  }
...
export const LicenseExpiryGuard = memo(({ children }: GuardProps) => {
  ...
  const isAdminCenterRoute = location.pathname.includes('/taskeye/admin-center');
  const isAccountDeletionRoute = location.pathname.includes('/taskeye/settings/account-deletion');
...
export const SetupGuard = memo(({ children }: GuardProps) => {
  ...
  if (!isSetupComplete) {
    return <Navigate to="/taskeye/setup" />;
  }
    ...
    >
      ...
      >
        <h1 style={{ fontSize: '24px', color: '#faad14', marginBottom: '16px' }}>
          Your TaskEye trial has expired!
        </h1>
        <p style={{ fontSize: '16px', color: '#555', marginBottom: '24px' }}>
          Please upgrade now to continue using TaskEye.
        </p>
        <button
          style={{
          ...
          onClick={() => (window.location.href = '/taskeye/admin-center/billing')}
        >
          Upgrade now
        </button>
      </div>
