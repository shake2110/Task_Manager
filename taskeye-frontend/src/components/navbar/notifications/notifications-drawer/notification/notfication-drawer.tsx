...
import { ITaskEyeNotification } from '@/types/notifications/notifications.types';
...
const NotificationDrawer = () => {
  ...
  const createPush = (message: string, title: string, teamId: string | null, url?: string) => {
    if (Notification.permission === 'granted' && showBrowserPush) {
      const img = 'https://taskeye.com/assets/icons/icon-128x128.png';
  ...
  const handleNotificationsUpdate = async (notification: ITaskEyeNotification) => {
    dispatch(fetchNotifications(notificationType));
    dispatch(fetchInvitations());

    if (isPushEnabled()) {
      const title = notification.team ? `${notification.team} | TaskEye` : 'TaskEye';
  ...
  const handleTeamInvitationsUpdate = async (data: ITeamInvitationViewModel) => {
    const notification: ITaskEyeNotification = {
      id: data.id || '',
      team: data.team_name || '',
  ...
  const goToUrl = async (event: React.MouseEvent, notification: ITaskEyeNotification) => {
