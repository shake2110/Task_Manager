...
import { evt_projects_create_click, evt_project_import_from_template_click } from '@/shared/taskeye-analytics-events';
...
const CreateProjectButton: React.FC<CreateProjectButtonProps> = ({ className }) => {
  ...
  const [selectedType, setSelectedType] = useState<'taskeye' | 'custom'>('taskeye');
  ...
  const createFromTaskEyeTemplate = async () => {
  ...
  const setCreatedProjectTemplate = async () => {
    ...
    try {
      setProjectImporting(true);
      if (selectedType === 'taskeye') {
        const res = await projectTemplatesApiService.createFromTaskEyeTemplate({
          template_id: currentTemplateId,
        });
        if (res.done) {
          navigate(
            `/taskeye/projects/${res.body.project_id}?tab=tasks-list&pinned_tab=tasks-list`
          );
        }
      } else {
