...
export const notificationsApiService = {
  getNotifications: async (filter: string): Promise<IServerResponse<ITaskEyeNotification[]>> => {
    const q = toQueryString({ filter });
    const response = await apiClient.get<IServerResponse<ITaskEyeNotification[]>>(
      `${rootUrl}${q}`
    );
