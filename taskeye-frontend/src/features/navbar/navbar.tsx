...
const Navbar = () => {
    ...
    () =>
      navRoutesList
        ...
        .map((route, index) => ({
          ...
          label: (
            <Link to={route.path} style={{ fontWeight: 800, textTransform: 'none', letterSpacing: '-0.03em', color: 'var(--flat-fg)' }}>
              {t(route.name)}
            </Link>
          ),
        })),
  ...
  useEffect(() => {
    const afterTaskEyeString = location.pathname.split('/taskeye/')[1];
    const pathKey = afterTaskEyeString.split('/')[0];
