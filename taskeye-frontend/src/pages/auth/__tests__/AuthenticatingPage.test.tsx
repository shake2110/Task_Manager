...
vi.mock('@/shared/constants', () => ({
  WORKLENZ_REDIRECT_PROJ_KEY: 'taskeye_redirect_proj',
}));
...
describe('AuthenticatingPage', () => {
  ...
  it('redirects to setup when user setup is not completed', async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/taskeye/setup');
  });
  ...
  it('redirects to home after successful authentication', async () => {
    ...
    expect(setSession).toHaveBeenCalledWith(mockUser);
    expect(setUser).toHaveBeenCalledWith(mockUser);
    expect(mockNavigate).toHaveBeenCalledWith('/taskeye/home');
  });

  it('redirects to project when redirect key is present in localStorage', async () => {
    const projectId = 'test-project-123';
    localStorage.setItem('taskeye_redirect_proj', projectId);
    ...
    expect(setSession).toHaveBeenCalledWith(mockUser);
    expect(setUser).toHaveBeenCalledWith(mockUser);
    expect(hrefValue).toBe(`/taskeye/projects/${projectId}?tab=tasks-list`);
    expect(localStorage.getItem('taskeye_redirect_proj')).toBeNull();
  });
