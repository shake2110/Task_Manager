...
const ReportingSider = () => {
        ...
        if (item.children) {
          return {
            ...
            children: item.children.map(child => ({
              key: child.key,
              label: <Link to={`/taskeye/reporting/${child.endpoint}`}>{t(`${child.name}`)}</Link>,
            })),
          };
        }
        return {
          key: item.key,
          label: <Link to={`/taskeye/reporting/${item.endpoint}`}>{t(`${item.name}`)}</Link>,
        };
      }),
    [t]
  ...
  const activeKey = useMemo(() => {
    const afterTaskEyeString = location.pathname?.split('/taskeye/reporting/')[1];
    return afterTaskEyeString?.split('/')[0];
  }, [location.pathname]);
